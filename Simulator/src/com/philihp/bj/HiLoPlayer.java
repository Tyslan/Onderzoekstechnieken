...
public class HiLoPlayer implements Player {
    ...
    public Response prompt(Hand playerHand, Hand dealerHand, boolean canSplit) {
        ...
        if (playerHand.isPair()) {
            response = PAIR_RESPONSE[playerHand.getShowCard().getTableOrdinal()][dealerHand.getShowCard().getTableOrdinal()];
        } else if (playerHand.isSoft()) {
            response = SOFT_RESPONSE[playerHand.getValue() - 13][dealerHand.getShowCard().getTableOrdinal()];
        } else {
            response = HARD_RESPONSE[playerHand.getValue() - 4][dealerHand.getShowCard().getTableOrdinal()];
        }
